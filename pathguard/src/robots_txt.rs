...
lazy_static! {
    pub static ref DEFAULT_ROBOTS: Box<str> = format!("User-agent: *\nDisallow: {dashboard}", dashboard=ARGS.dashboard).into_boxed_str();
}
...
fn update_or(robots: &str, default: &'static str, dashboard: &'static str) -> Cow<'static, str> {
    ...
    while let Some(line) = lines.next() {
        if
            line.trim().to_lowercase().starts_with("user-agent")
        {
...
mod tests {
    ...
    fn robots() {
        ...
        }
        {
            ...
            assert_eq!(update(INPUT), formatcp!("User-agent: amogus\nDisallow: {DASHBOARD}\nDisallow: /foo\n\n{DEFAULT_ROBOTS}"));
        }
        {
            const INPUT: &str = "UsEr-AgEnT: *\nDisallow: /foo";
            assert_eq!(update(INPUT), formatcp!("User-agent: *\nDisallow: {DASHBOARD}\nDisallow: /foo"))
        }
    }
}